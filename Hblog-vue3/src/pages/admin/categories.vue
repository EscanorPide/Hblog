...
defineOptions({ name: 'AdminCategories' })
...
const addRules = {
  name: [
    ...
    { min: 1, max: 10, message: '分类名称字数限制 1 ~ 10 之间', trigger: 'blur' },
  ],
}
...
function fetchList() {
  ...
  getCategoryPageList(buildQuery())
    .then((res) => {
      if (res.success === false) {
        showMessage(res.message || '获取分类列表失败', 'error')
        return
      }
...
function handleAdd() {
  ...
  addCategory({ name: addForm.name.trim() })
    .then((res) => {
      if (res.success === false) {
        showMessage(res.message || '新增分类失败', 'error')
        return
      }
      showMessage('新增成功')
...
function handleEdit() {
  ...
  submitLoading.value = true
  updateCategory({ id: editForm.id, name: editForm.name.trim() })
    .then((res) => {
      if (res.success === false) {
        showMessage(res.message || '修改分类失败', 'error')
        return
      }
      showMessage('修改成功')
...
function handleDelete(row) {
  showModel(`确认删除分类「${row.name}」吗？`, 'warning', '删除确认')
    .then(() => {
